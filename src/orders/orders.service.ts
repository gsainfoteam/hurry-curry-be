...
export class OrdersService {
  ...
  async create(createOrderDto: CreateOrderDto, userId: number) {
    ...
    // 2. 마지막 주문 종료 시간 가져오기 (DB 담당자의 역할이지만 임시 함수 사용)
    const lastOrderEndTime: Date = await this.getLastOrderEndTime(); 
    
    // 3. pickupTime 계산 (핵심 로직: Time = max(current_time, last_order_end_time) + quantity*3min)
    const now = new Date();
    // 현재 시간과 마지막 주문 종료 시간 중 더 늦은 시간을 시작 시간으로 잡습니다.
    const startTime = (now > lastOrderEndTime) ? now : lastOrderEndTime; 
    
    // 픽업 시간을 계산 (totalQuantity * 3분)
    // Date.getTime()은 밀리초(ms) 단위이므로 3 * 60000 (ms)를 곱합니다.
    const pickupTime = new Date(startTime.getTime() + totalQuantity * 3 * 60000);

    // 4. DB 담당자에게 전달할 데이터 (현재는 임시 큐에 저장)
    const orderToSave = {
    ...
    // **[DB 담당자 역할]:** 이 데이터를 DB에 저장하는 함수를 호출해야 합니다.
...
// 2. 주문 건수 조회 로직 (READ)
  async getActiveOrderCount(): Promise<number> {
      // **[DB 담당자 역할]:** DB에서 'PREPARING' 또는 'CONFIRMED' 상태의 주문 건수를 조회해야 합니다.
      return mockOrderQueue.length; // 임시 큐의 크기를 반환
  }

  // 3. 임시: 마지막 주문 종료 시간을 반환하는 함수 (DB 담당자의 함수를 가정)
